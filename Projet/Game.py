...
if carte == 1:
    listeEnnemis = [["soldat1","soldat1", "soldat1","soldat2"],
...
while running : # boucle infinie pour laisser la fenêtre ouverte
    ...
    for event in pygame.event.get(): # parcours de tous les event pygame dans cette fenêtre
        ...
        if k[pygame.K_a] and argent >= 40: # si la touche a a été tapée
            if k[pygame.K_1] and build1.construire():
                tour = Tours.Tour(build1.x,build1.y,1,1)
                argent -= 40
                towers[0] = tour
            if k[pygame.K_2] and build2.construire():
                tour = Tours.Tour(build2.x,build2.y,1,2)
                argent -= 40
                towers[1] = tour
            if k[pygame.K_3] and build3.construire():
                tour = Tours.Tour(build3.x,build3.y,1,3)
                argent -= 40
                towers[2] = tour
            if k[pygame.K_4] and build4.construire():
                tour = Tours.Tour(build4.x,build4.y,1,4)
                argent -= 40
                towers[3] = tour
            if k[pygame.K_5] and build5.construire():
                tour = Tours.Tour(build5.x,build5.y,1,5)
                argent -= 40
                towers[4] = tour
            if k[pygame.K_6] and build6.construire():
                tour = Tours.Tour(build6.x,build6.y,1,6)
                argent -= 40
                towers[5] = tour
            if k[pygame.K_7] and build7.construire():
                tour = Tours.Tour(build7.x,build7.y,1,7)
                argent -= 40
                towers[6] = tour
            if k[pygame.K_8] and build8.construire():
                tour = Tours.Tour(build8.x,build8.y,1,8)
                argent -= 40
                towers[7] = tour
            if k[pygame.K_9] and build9.construire():
                tour = Tours.Tour(build9.x,build9.y,1,9)
                argent -= 40
                towers[8] = tour
                
        if k[pygame.K_z] and argent >= 60: # si la touche z a été tapée
            if k[pygame.K_1] and build1.construire():
        ...
        if k[pygame.K_e] and argent >= 70: # si la touche e a été tapée
            if k[pygame.K_1] and build1.construire():
        ...
        if k[pygame.K_r] and argent >= 150: # si la touche r a été tapée
            if k[pygame.K_1] and build1.construire():
    ...
    for tour in towers:
        if tour != 0:
            ...
            screen.blit(tour.image,[tour.x,tour.y])
